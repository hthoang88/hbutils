...
- (void)addAttributesFromDictionary:(NSDictionary *)attributesToKeyPath {
    for (NSString *key in attributesToKeyPath) {
        if ([key rangeOfString:@"HBUtilsType_"].location != NSNotFound) {
            continue;
        }
        id value = attributesToKeyPath[key];
        FEMAttribute *att = [FEMAttribute mappingOfProperty:key toKeyPath:value];
        NSString *type = attributesToKeyPath[[NSString stringWithFormat:@"HBUtilsType_%@", key]];
        if (type) {
            att.type = type;
        }
